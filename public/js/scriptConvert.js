const fromCurrencyOptions = document.querySelector('.from-currency input');
const toCurrencyOptions = document.querySelector('.to-currency input');
const fromAmount = document.querySelector('.from-amount input');
const fromResult = document.getElementById('from-result');
const toResult = document.getElementById('converted_amount');
const toInput = document.getElementById('id-input');
...
function displayConvertedData(fromCurrency, toCurrency, fromAmt, toAmt){
    ...
    toInput.value = `${toAmt.toFixed(2)} ${toCurrency}`;
}
